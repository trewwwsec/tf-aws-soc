...
class AnthropicClient(BaseLLMClient):
    ...
    def __init__(self, model: str = "claude-3-sonnet-20240229", temperature: float = 0.3):
        self.model = model
        self.temperature = temperature
...
class OllamaClient(BaseLLMClient):
    ...
    def _mock_response(self, prompt: str) -> str:
        """Return a mock response when Ollama is not available."""
        return json.dumps({
            "title": "Security Alert Detected",
            "summary": "A security event was detected that requires investigation.",
            "investigation_steps": [
...
"""
    ...
    def _fallback_analysis(
        ...
        if severity >= 12:
            ...
            recommended_actions.append(f"[IMMEDIATE] Investigate alert within 30 minutes")
